...
Loop = 0
while Loop == 0:
    required_date = [int(required_date) for required_date in input("Enter the date and time of epoch (yyyy,mm,dd,h,m). Note: the epoch has recorded in zero second (0.000):\n").split()]
    if required_date[0]>2020:
        print("Warning: The year should not be bigger than 2020. You wrote","\033[1;31;42m",required_date[0],"\033[0m")
        ...
        print("Warning: The minute should be selected as 0, 15, 30 or 45. You wrote","\033[1;31;42m",required_date[4],"\033[0m")
    else:
        Loop = 1        
"""
...
for i in range(len(read_file_02o)):
    if set(["APPROX"]).intersection(set(read_file_02o[i])):
...
if len(r) != 0:
    print("The satellite","\033[1;32;41m",', '.join(map(str, satellites_coord[r,0].astype(int))),"\033[0m","has error on clock bias and it is removed from calculations!")
...
for i in range(3):
    ...
    pdop[0,i] = np.sqrt(var_cov_topocentric[3*i,3*i] + var_cov_topocentric[3*i+1,3*i+1]\
                      + var_cov_topocentric[3*i+2,3*i+2])
    tdop[0,i] = np.sqrt(var_cov[3,3])                  
    gdop[0,i] = np.sqrt(var_cov_topocentric[3*i,3*i] + var_cov_topocentric[3*i+1,3*i+1]\
                      + var_cov_topocentric[3*i+2,3*i+2] + var_cov[3,3])        
"""
...
for i in range(num_sat):
    for j in range(3):
        range_dist[i,j] = np.sqrt(topo_coord[3*j,i]**2 + \
                         topo_coord[3*j+1,i]**2 + topo_coord[3*j+2,i]**2)
        elevation_angle[i,j] = np.pi/2 - \
                         np.arccos(topo_coord[3*j+2,i]/range_dist[i,j])
        azimuth[i,j] = np.arctan(topo_coord[3*j+1,i]/topo_coord[3*j,i])
        if topo_coord[3*j+1,i]<0:
            azimuth[i,j] = azimuth[i,j] + np.pi
        elif topo_coord[3*j,i]<0 and topo_coord[3*j+1,i]>0:
